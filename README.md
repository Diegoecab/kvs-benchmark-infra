...
The nine load generators expose no ingress. OCI NoSQL runners use distinct private VNIC addresses and the Oracle Services Network. Each ADB API runner remains private but has an isolated VCN and dedicated NAT so the service observes three independent outbound identities. AWS runners retain independently controlled identities. The benchmark distributes, rather than multiplies, the aggregate offered load across three source VMs per target. AWS Systems Manager and OCI Compute Run Command remain the only control paths.
